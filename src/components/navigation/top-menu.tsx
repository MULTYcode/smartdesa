      ...
      <li className="relative">
        <button
          ...
          className={`inline-flex items-center rounded-sm px-3 py-2 text-sm outline-none transition-colors duration-150
                      ...
                      ${
                        open || isActiveBtn
                          ? "bg-secondary text-secondary-foreground border-b-2 border-[#0d6b3f]"
                          : "border border-transparent hover:bg-secondary hover:text-secondary-foreground"
                      }`}
          data-open={open ? "true" : "false"}
          aria-haspopup="menu"
...
export function NavMenu({
  ...
  return (
    <nav aria-label="Primary" className="w-full bg-background text-foreground">
      <div className="mx-auto max-w-6xl">
        <FloatingTree>
          <ul role="menubar" className="flex w-full items-center gap-1 overflow-x-auto py-2">
            {top.map((it) => {
              ...
              return <TopMenuTrigger key={it.title} item={it} />
            })}
          </ul>
        </FloatingTree>
      </div>
